...
void NetworkCommunication::getRequest(QByteArray postData)
{
    Q_UNUSED(postData);
    qDebug()<<Q_FUNC_INFO<<endl;
    N_request.setUrl(QUrl(this->getRouteURL()));
    N_request.setHeader(QNetworkRequest::ContentTypeHeader,QVariant(QString("application/json")));
//    connect(NA_manager,SIGNAL(finished(QNetworkReply*)),this,SLOT(receivePacket(QNetworkReply*)));
//    NA_manager->get(QNetworkRequest(QUrl(this->getRouteURL())));
